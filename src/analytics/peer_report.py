...
class PeerComparisonReport:
    """Generate formatted Excel reports for peer comparisons."""
    ...
    def __init__(self):
        ...
        self.output_file = "output/peer_comparison.xlsx"

    def load_data(self):
        """Load peer comparison data from SQLite."""

        ratios = pd.read_sql("SELECT * FROM financial_ratios", self.conn)

        peers = pd.read_sql("SELECT * FROM peer_groups", self.conn)

        percentiles = pd.read_sql("SELECT * FROM peer_percentiles", self.conn)
    ...
    def build_percentile_table(self, percentiles):
        """Pivot percentile rankings into a report table."""
    ...
    def generate_report(self):
        ...
        percentile_table = self.build_percentile_table(percentiles)

        df = df.merge(percentile_table, on="company_id", how="left")
        ...
        for group in peer_groups:
            ...
            peer_df.to_excel(writer, sheet_name=group[:31], index=False)
    ...
    def format_workbook(self):
        ...
        yellow_fill = PatternFill(fill_type="solid", start_color="FFF2CC")

        red_fill = PatternFill(fill_type="solid", start_color="F4CCCC")
        ...
        for sheet in wb.worksheets:
            ...
            for idx, header in enumerate(headers, start=1):
                ...
                if (
                    header is not None
                    and header != "company_id"
                    and (
                        "return_on_equity_pct" in str(header)
                        or "return_on_capital_employed_pct" in str(header)
                        or "net_profit_margin_pct" in str(header)
                        or "debt_to_equity" in str(header)
                        or "free_cash_flow_cr" in str(header)
                        or "pat_cagr_5yr" in str(header)
                        or "revenue_cagr_5yr" in str(header)
                        or "eps_cagr_5yr" in str(header)
                        or "interest_coverage" in str(header)
                        or "asset_turnover" in str(header)
                    )
                ):
            ...
            if "is_benchmark" in headers:
                benchmark_column = headers.index("is_benchmark") + 1

            for row in range(2, sheet.max_row + 1):

                if benchmark_column:

                    if sheet.cell(row, benchmark_column).value == 1:

                        for col in range(1, sheet.max_column + 1):
                            sheet.cell(row, col).fill = gold_fill
            ...
            sheet.cell(median_row, 1).value = "Peer Median"
            ...
            for col in range(2, sheet.max_column + 1):
                ...
                if values:
                    ...
                    sheet.cell(median_row, col).value = round(
...
def main():
    """Generate and format the peer comparison report."""
