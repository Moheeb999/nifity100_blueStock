...
def normalize_columns(df):
    """Normalize DataFrame column names for consistent processing."""
...
def validate_composite_pk(df, table_name):
    """Validate uniqueness of the company-year composite key."""
    failures = []
...
def validate_fk(df, companies_df, table_name):
    """Validate foreign key references against the companies table."""
    failures = []
...
def validate_balance_sheet(bs_df):
    """Validate the balance sheet accounting equation."""
    failures = []
    invalid_rows = bs_df[
        (
...
def validate_opm(pnl_df):
    """Validate operating profit margin calculations."""
    failures = []
...
def validate_positive_sales(pnl_df):
    """Validate that sales values are non-negative."""
    failures = []
...
def validate_net_cash(cf_df):
    """Validate net cash flow calculations."""
    failures = []
    calc_net = (
...
def validate_tax_rate(pnl_df):
    """Validate tax percentages are within valid bounds."""
    failures = []
    invalid_rows = pnl_df[
        (pnl_df["tax_percentage"] < 0) | (pnl_df["tax_percentage"] > 100)
    ]
...
def validate_dividend_cap(pnl_df):
    """Validate dividend payout percentages."""
    failures = []
...
def validate_urls(companies_df):
    """Validate company website and profile URLs."""
    failures = []
...
def validate_eps_sign(pnl_df):
    """Validate EPS values against net profit signs."""
    failures = []
    invalid_rows = pnl_df[(pnl_df["net_profit"] < 0) & (pnl_df["eps"] > 0)]
...
def validate_profiles(companies_df):
    """Validate the availability of NSE and BSE profile links."""
    failures = []
    invalid_rows = companies_df[
        companies_df["nse_profile"].isna() | companies_df["bse_profile"].isna()
    ]
...
def validate_year_coverage(pnl_df):
    """Validate sufficient yearly financial history for each company."""
    failures = []
...
def validate_annual_reports(doc_df):
    """Validate the availability of annual report documents."""
    failures = []
...
def validate_financial_ratios(ratio_df):
    """Validate required financial ratio fields."""
    failures = []
...
def main():
    """Run all ETL data quality validation checks and generate the validation report."""
    ...
    financial_ratios = normalize_columns(
        pd.read_excel("data/raw/financial_ratios.xlsx")
    )
