...
class ScreenerEngine:
    ...
    def load_data(self):
        ...
        market = pd.read_sql("SELECT * FROM market_cap", self.conn)
        ...
        df = df.merge(pnl, on=["company_id", "year"], how="left")
        ...
        df = df.groupby("company_id").tail(1).reset_index(drop=True)
    ...
    def apply_preset(self, df, preset_name):
        ...
        if preset is None:
            raise ValueError(f"Preset '{preset_name}' is missing or empty.")

        # ----------------------------
        # ROE
        # ----------------------------
        if "roe_min" in preset:
            df = df[df["return_on_equity_pct"] >= preset["roe_min"]]
        ...
        if "debt_to_equity_max" in preset:
            ...
            non_financial = (df["broad_sector"] != "Financials") & (
                df["debt_to_equity"] <= preset["debt_to_equity_max"]
            )

            financial = df["broad_sector"] == "Financials"

            df = df[non_financial | financial]

        # ----------------------------
        # Free Cash Flow
        # ----------------------------
        if "free_cash_flow_min" in preset:
            df = df[df["free_cash_flow_cr"] >= preset["free_cash_flow_min"]]

        # ----------------------------
        # Revenue CAGR
        # ----------------------------
        if "revenue_cagr_5yr_min" in preset:
            df = df[df["revenue_cagr_5yr"] >= preset["revenue_cagr_5yr_min"]]

        # ----------------------------
        # PAT CAGR
        # ----------------------------
        if "pat_cagr_5yr_min" in preset:
            df = df[df["pat_cagr_5yr"] >= preset["pat_cagr_5yr_min"]]
        ...
        if "dividend_yield_min" in preset:
            df = df[
                ...
                | (df["dividend_yield_pct"] >= preset["dividend_yield_min"])
            ]
        ...
        # ----------------------------
        if "pe_max" in preset:
            df = df[df["pe_ratio"].isna() | (df["pe_ratio"] <= preset["pe_max"])]

        # ----------------------------
        # PB Ratio
        # ----------------------------
        if "pb_max" in preset:
            df = df[df["pb_ratio"].isna() | (df["pb_ratio"] <= preset["pb_max"])]

        # ----------------------------
        # Sales
        # ----------------------------
        if "sales_min" in preset:
            df = df[df["sales"] >= preset["sales_min"]]

        # ----------------------------
        # Dividend Payout
        # ----------------------------
        if "dividend_payout_max" in preset:
            df = df[df["dividend_payout_ratio_pct"] <= preset["dividend_payout_max"]]
